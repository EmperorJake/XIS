...
def initialize(__loader, macros, nothing, template):
    ...
    def render(__stream, econtext, rcontext, __i18n_domain=None, __i18n_context=None):
        ...
        try:
            ...
            __backup_incompatible_grf_139703132008120 = get('incompatible_grf', __marker)

            # <Value 'incompatible_grfs' (14:48)> -> __iterator
            __token = 503
            __iterator = getitem('incompatible_grfs')
            (__iterator, ____index_139703105970416, ) = getitem('repeat')('incompatible_grf', __iterator)
            econtext['incompatible_grf'] = None
            for __item in __iterator:
                econtext['incompatible_grf'] = __item

                # <Interpolation value=<Substitution '\n    if (grf_future_status("${incompatible_grf.grfid}")) {\n        error(FATAL, string(STR_ERR_INCOMPATIBLE_SET, "${incompatible_grf.grfname}"));\n    }\n' (14:67)> braces_required=True translation=False at 7f0f2a0163c8> -> __content_139703124559104
                __token = 527
                __token = 552
                __content_139703124559104 = _lookup_attr(getitem('incompatible_grf'), 'grfid')
                __content_139703124559104 = __quote(__content_139703124559104, '\x00', '&#0;', None, False)
                __token = 638
                __content_139703124559104_636 = _lookup_attr(getitem('incompatible_grf'), 'grfname')
                __content_139703124559104_636 = __quote(__content_139703124559104_636, '\x00', '&#0;', None, False)
                ...
                ____index_139703105970416 -= 1
                if (____index_139703105970416 > 0):
                    __append('')
            if (__backup_incompatible_grf_139703132008120 is __marker):
                del econtext['incompatible_grf']
            else:
                econtext['incompatible_grf'] = __backup_incompatible_grf_139703132008120
            __append('\n\n\n')
