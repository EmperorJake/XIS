# -*- coding: utf-8 -*-
__filename = '/mnt/c/Users/Jakob/Games/GitHub/XIS/src/templates/checks.pynml'
...
def initialize(__loader, macros, nothing, template):
    ...
    def render(__stream, econtext, rcontext, __i18n_domain=None, __i18n_context=None):
        ...
        try:
            ...
            # <Static value=<_ast.Dict object at 0x7f176390cc40> name=None at 7f176390ca60> -> __attrs_139738428980048
            __attrs_139738428980048 = _static_139738431409216
            __backup_incompatible_grf_139738447011024 = get('incompatible_grf', __marker)

            # <Value 'incompatible_grfs' (14:48)> -> __iterator
            __token = 503
            __iterator = getitem('incompatible_grfs')
            (__iterator, ____index_139738428980240, ) = getitem('repeat')('incompatible_grf', __iterator)
            econtext['incompatible_grf'] = None
            for __item in __iterator:
                econtext['incompatible_grf'] = __item

                # <Interpolation value=<Substitution '\n    if (grf_future_status("${incompatible_grf.grfid}")) {\n        error(FATAL, string(STR_ERR_INCOMPATIBLE_SET, "${incompatible_grf.grfname}"));\n    }\n' (14:67)> braces_required=True translation=False default='"?"' default_marker='"?"' at 7f17636bbd60> -> __content_139738435934384
                __token = 527
                __token = 552
                __content_139738435934384 = _lookup_attr(getitem('incompatible_grf'), 'grfid')
                __content_139738435934384 = __quote(__content_139738435934384, '\x00', '&#0;', None, None)
                __token = 638
                __content_139738435934384_636 = _lookup_attr(getitem('incompatible_grf'), 'grfname')
                __content_139738435934384_636 = __quote(__content_139738435934384_636, '\x00', '&#0;', None, None)
                __content_139738435934384 = ('%s%s%s%s%s' % ('\n    if (grf_future_status("', (__content_139738435934384 if (__content_139738435934384 is not None) else ''), '")) {\n        error(FATAL, string(STR_ERR_INCOMPATIBLE_SET, "', (__content_139738435934384_636 if (__content_139738435934384_636 is not None) else ''), '"));\n    }\n', ))
                ...
                ____index_139738428980240 -= 1
                if (____index_139738428980240 > 0):
                    __append('')
            if (__backup_incompatible_grf_139738447011024 is __marker):
                del econtext['incompatible_grf']
            else:
                econtext['incompatible_grf'] = __backup_incompatible_grf_139738447011024
