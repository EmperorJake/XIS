# -*- coding: utf-8 -*-
__filename = '/home/jake/GRF/firs-3.0.10-source/bundle_dir/src/src/templates/checks.pynml'
...
def initialize(__loader, macros, nothing, template):
    ...
    def render(__stream, econtext, rcontext, __i18n_domain=None, __i18n_context=None):
        ...
        try:
            ...
            __backup_incompatible_grf_140183257340336 = get('incompatible_grf', __marker)

            # <Value 'incompatible_grfs' (14:48)> -> __iterator
            __token = 503
            __iterator = getitem('incompatible_grfs')
            (__iterator, ____index_140183231371808, ) = getitem('repeat')('incompatible_grf', __iterator)
            econtext['incompatible_grf'] = None
            for __item in __iterator:
                econtext['incompatible_grf'] = __item

                # <Interpolation value=<Substitution '\n    if (grf_future_status("${incompatible_grf.grfid}")) {\n        error(FATAL, string(STR_ERR_INCOMPATIBLE_SET, "${incompatible_grf.grfname}"));\n    }\n' (14:67)> braces_required=True translation=False at 7f7ef3b596a0> -> __content_140183249773656
                __token = 527
                __token = 552
                __content_140183249773656 = _lookup_attr(getitem('incompatible_grf'), 'grfid')
                __content_140183249773656 = __quote(__content_140183249773656, '\x00', '&#0;', None, False)
                __token = 638
                __content_140183249773656_636 = _lookup_attr(getitem('incompatible_grf'), 'grfname')
                __content_140183249773656_636 = __quote(__content_140183249773656_636, '\x00', '&#0;', None, False)
                __content_140183249773656 = ('%s%s%s%s%s' % ('\n    if (grf_future_status("', (__content_140183249773656 if (__content_140183249773656 is not None) else ''), '")) {\n        error(FATAL, string(STR_ERR_INCOMPATIBLE_SET, "', (__content_140183249773656_636 if (__content_140183249773656_636 is not None) else ''), '"));\n    }\n', ))
                ...
                ____index_140183231371808 -= 1
                if (____index_140183231371808 > 0):
                    __append('')
            if (__backup_incompatible_grf_140183257340336 is __marker):
                del econtext['incompatible_grf']
            else:
                econtext['incompatible_grf'] = __backup_incompatible_grf_140183257340336
            __append('\n\n\n')
