...
def initialize(__loader, macros, nothing, template):
    ...
    def render(__stream, econtext, rcontext, __i18n_domain=None, __i18n_context=None):
        ...
        try:
            ...
            __backup_incompatible_grf_139913576272728 = get('incompatible_grf', __marker)

            # <Value 'incompatible_grfs' (14:48)> -> __iterator
            __token = 503
            __iterator = getitem('incompatible_grfs')
            (__iterator, ____index_139913550419560, ) = getitem('repeat')('incompatible_grf', __iterator)
            econtext['incompatible_grf'] = None
            for __item in __iterator:
                econtext['incompatible_grf'] = __item

                # <Interpolation value=<Substitution '\n    if (grf_future_status("${incompatible_grf.grfid}")) {\n        error(FATAL, string(STR_ERR_INCOMPATIBLE_SET, "${incompatible_grf.grfname}"));\n    }\n' (14:67)> braces_required=True translation=False at 7f402978d748> -> __content_139913568826456
                __token = 527
                __token = 552
                __content_139913568826456 = _lookup_attr(getitem('incompatible_grf'), 'grfid')
                __content_139913568826456 = __quote(__content_139913568826456, '\x00', '&#0;', None, False)
                __token = 638
                __content_139913568826456_636 = _lookup_attr(getitem('incompatible_grf'), 'grfname')
                __content_139913568826456_636 = __quote(__content_139913568826456_636, '\x00', '&#0;', None, False)
                __content_139913568826456 = ('%s%s%s%s%s' % ('\n    if (grf_future_status("', (__content_139913568826456 if (__content_139913568826456 is not None) else ''), '")) {\n        error(FATAL, string(STR_ERR_INCOMPATIBLE_SET, "', (__content_139913568826456_636 if (__content_139913568826456_636 is not None) else ''), '"));\n    }\n', ))
                ...
                ____index_139913550419560 -= 1
                if (____index_139913550419560 > 0):
                    __append('')
            if (__backup_incompatible_grf_139913576272728 is __marker):
                del econtext['incompatible_grf']
            else:
                econtext['incompatible_grf'] = __backup_incompatible_grf_139913576272728
            __append('\n\n\n')
