...
def initialize(__loader, macros, nothing, template):
    ...
    def render(__stream, econtext, rcontext, __i18n_domain=None, __i18n_context=None):
        ...
        try:
            ...
            __backup_economy_140383663151760 = get('economy', __marker)

            # <Value 'economies' (21:35)> -> __iterator
            __token = 1252
            __iterator = getitem('economies')
            (__iterator, ____index_140383637160568, ) = getitem('repeat')('economy', __iterator)
            econtext['economy'] = None
            for __item in __iterator:
                econtext['economy'] = __item

                # <Interpolation value=<Substitution '\n    if (economy_selection == ${repeat.economy.index}) {\n        economy = ${economy.numeric_id};\n    }\n' (21:46)> braces_required=True translation=False at 7fad9cd340f0> -> __content_140383655711776
                __token = 1268
                __token = 1295
                ...
                __token = 1340
                __content_140383655711776_1338 = _lookup_attr(getitem('economy'), 'numeric_id')
                __content_140383655711776_1338 = __quote(__content_140383655711776_1338, '\x00', '&#0;', None, False)
                __content_140383655711776 = ('%s%s%s%s%s' % ('\n    if (economy_selection == ', (__content_140383655711776 if (__content_140383655711776 is not None) else ''), ') {\n        economy = ', (__content_140383655711776_1338 if (__content_140383655711776_1338 is not None) else ''), ';\n    }\n', ))
                ...
                ____index_140383637160568 -= 1
                if (____index_140383637160568 > 0):
                    __append('')
            if (__backup_economy_140383663151760 is __marker):
                del econtext['economy']
            else:
                econtext['economy'] = __backup_economy_140383663151760
